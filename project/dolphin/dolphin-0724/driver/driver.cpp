...
#include "command/command_factory_inst.hpp"
#include "command/monitor_command.hpp"
#include "command/task_command.hpp"

bool flag_parse(std::string& str_command)
{
    size_t pos = str_command.find("||");
    if (pos == std::string::npos || pos == 0)
        return false;

    int fd = std::stoi(str_command.substr(0,pos));
    std::string info{str_command.substr(pos+2)};
...
bool Driver::start_driver()
{
    ...
    while (1)
    {
        client_fd = accept(socket_fd_,(struct sockaddr*)& client_addr,&addrlen);
        if (client_fd == -1)
        {
            log_msg = "accept failed!";
            Log::instance().log(log_msg);
            continue;
        }
        
        log_msg = "connect from " + std::string(inet_ntoa(client_addr.sin_addr)) + ':' + std::to_string(ntohs(client_addr.sin_port));
        Log::instance().log(log_msg);
