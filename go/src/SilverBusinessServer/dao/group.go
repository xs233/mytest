...
//------------------------------------------------------------------------------------------
//导入设备列表到数据库中
func ImportDeviceToList(deviceList []lib.Device) error {
	session, err := MySQL.DB.Begin()
	if err != nil {
		return err
	}
...
//register live devices information to the database
func RegisterDeviceToDB(registerInfo lib.RegisterLiveDevices) (status int, err error) {
	session, err := MySQL.DB.Begin()
	if err != nil {
		return lib.RegisterFailed, err
	}

	var deviceID int64
	if status, deviceID, err = MySQL.RegisterInfoToDeviceTable(session, registerInfo); err != nil {
		session.Rollback()
		return status, err
	} else {
		if err = MySQL.RegisterInfoToAlgTable(session, registerInfo, deviceID); err != nil {
			session.Rollback()
			return lib.RegisterFailed, err
		}
	}
