...
func GetSequenceIDByName(name string) (id int64, err error) {
	...
	sql := "select current_value, increment_value from imp_t_sequence where sequence_name = ?"
	...
	sql = "update imp_t_sequence set current_value = current_value + increment_value where sequence_name = ?"
