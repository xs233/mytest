...
func binds() {
	...
	if env.Get("httpserver.module.v1.api").(bool) { // 注释后面的OK，标示的是测试代码正常返回成功
		...
		V1Router.POST("/api/livedevices", v1api.HandleRegisterLiveDevicesPost)							    					 //注册智能设备
		...
		V1Router.POST("/api/alarms", v1api.HandlePushAlarmsPost)				   							   //报警推送和保存
		...
		V1Router.GET("/api/alg/unmonitor/devices", MiddleWareCheckUserHasLogin(), v1api.HandleUnmonitorDeviceGet)  //查询未进行算法分析的普通相机
		V1Router.GET("/api/alg/devices/:did", MiddleWareCheckUserHasLogin(), v1api.HandleQueryAlgConfigGet)        //查询特定设备的算法配置      
		...
		V1Router.GET("/api/alg/status", MiddleWareCheckUserHasLogin(), v1api.HandleQueryAllAlgDevicesStatusGet)       //客户端定时查询所有设备的算法运行状况，定时15秒
