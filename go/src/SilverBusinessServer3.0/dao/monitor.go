...
func QueryDeviceNumber(keyword string) (number int, err error) {
	session, err := sql.DB.Begin()
	if err != nil {
		return -1, err
	}

	number, err = sql.QueryDeviceNumber(session, keyword)
	if err != nil {
		session.Rollback()
		return -1, err
	}
...
func QueryDeviceListByPage(keyword string, offset, count int64) (devices []lib.Device, err error) {
	session, err := sql.DB.Begin()
	if err != nil {
		return nil, err
	}

	devices, err = sql.QueryDeviceListByPage(session, keyword, offset, count)
	if err != nil {
		session.Rollback()
		return nil, err
	}
...
//--------------------------------------------------------------------------
// 添加摄像机
func AddConcentrateDeviceByDeviceidList(deviceidList []int64) (err error) {
	session, err := sql.DB.Begin()
	if err != nil {
		return err
	}
	err = sql.AddDeviceToAlg(session, deviceidList)
	if err != nil {
		session.Rollback()
		return err
	}
...
//---------------------------------------------------------------------------
//查询所有算法分析设备
func QueryAllAlgs() (deviceList []lib.AlgDevice, err error) {
	session, err := sql.DB.Begin()
	if err != nil {
		return nil, err
	}

	deviceList, err = sql.QueryAllAlgs(session)
	if err != nil {
		session.Rollback()
		return nil, err
	}
...
func GetAlgconfig(deviceID int64) (algConfig string, err error) {
	session, err := sql.DB.Begin()
	if err != nil {
		return "", err
	}

	if algConfig, err = sql.GetAlgconfig(session, deviceID); err != nil {
		session.Rollback()
		return "", err
	}
